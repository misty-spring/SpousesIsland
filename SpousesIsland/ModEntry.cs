...
namespace SpousesIsland
{
    ...
    {
        // Entry: mod entry point, called after the mod is first loaded.
        // helper: Provides simplified APIs for writing mods.
        ...
        public static Dictionary<string, ContentPackData> CustomSchedule = new Dictionary<string, ContentPackData>();
        
        //events
        private void OnGameLaunched(object sender, GameLaunchedEventArgs e)
        {
            
            CanEditFishShop = true;
            ...
            if (DaysOfWeek.Count is not 6)
            {
                this.Monitor.Log("Resetting DaysOfWeek...", LogLevel.Trace);
                DaysOfWeek.Add("Tue");
                DaysOfWeek.Add("Wed");
                DaysOfWeek.Add("Thu");
                DaysOfWeek.Add("Fri");
            ...
            foreach (IContentPack contentPack in this.Helper.ContentPacks.GetOwned())
            {
                ...
                foreach (var cpd in obj.data)
                {
                    ...
                    //warn logs
                    if ((cpd.Location1.Time >= cpd.Location2.Time) || (cpd.Location1.Time >= cpd.Location3.Time) || (cpd.Location2.Time >= cpd.Location3.Time))
                    {
                        if (cpd.Location1?.Time >= cpd.Location2?.Time)
                        {
                            this.Monitor.Log($"Location1's Time ({cpd.Location1.Time}) conflicts with Location2's Time ({cpd.Location2.Time})! This will cause errors. (Make sure the values aren't the same, and that Time1 happens before Time2.)", LogLevel.Warn);
                        }
                        if (cpd.Location1?.Time >= cpd.Location3?.Time)
                        {
                            this.Monitor.Log($"Location1's Time ({cpd.Location1.Time}) conflicts with Location3's Time ({cpd.Location3.Time})! This will cause errors. (Make sure the values aren't the same, and that Time1 happens before Time3.)", LogLevel.Warn);
                        }
                        if (cpd.Location2?.Time >= cpd.Location3?.Time)
                        {
                            this.Monitor.Log($"Location2's Time ({cpd.Location2.Time}) conflicts with Location3's Time ({cpd.Location3.Time})! This will cause errors. (Make sure the values aren't the same, and that Time2 happens before Time3.)", LogLevel.Warn);
                        }
                        this.Monitor.Log($"One or more fields in '{contentPack.Manifest.Name}' conflict with each other.", LogLevel.Warn);
                    }
                    //error logs
        ...
        {
            if (e.Name.IsEquivalentTo("Maps/IslandFarmHouse") && Config.Allow_Children == true && Config.CustomRoom == false && IntKids >= 1)
            {
                e.LoadFromModFile<Map>("assets/Maps/FarmHouse_kid.tbin", AssetLoadPriority.Medium);
            };
            if (e.Name.IsEquivalentTo("Maps/IslandFarmHouse") && Config.Allow_Children == true && Config.CustomRoom == true && IntKids >= 1)
            {
                e.LoadFromModFile<Map>("assets/Maps/FarmHouse_kid_custom.tbin", AssetLoadPriority.Medium);
            };
            if (e.Name.IsEquivalentTo("Maps/IslandFarmHouse") && Config.Allow_Children == true && IntKids >= 1)
                e.Edit(asset =>
                {
            ...
            if (e.Name.IsEquivalentTo("Maps/Island_S"))
                e.Edit(asset => {
                    ...
                    map.Properties.Add("NPCWarp", "17 44 FishShop 4 4 36 11 IslandEast 0 45 36 12 IslandEast 0 46 36 13 IslandEast 0 47 -1 11 IslandWest 105 40 -1 10 IslandWest 105 40 -1 12 IslandWest 105 40 -1 13 IslandWest 105 40 17 -1 IslandNorth 35 89 18 -1 IslandNorth 36 89 19 -1 IslandNorth 37 89 27 -1 IslandNorth 43 89 28 -1 IslandNorth 43 89 43 28 IslandSouthEast 0 29 43 29 IslandSouthEast 0 29 43 30 IslandSouthEast 0 29");
                });
            ...
            if (e.Name.IsEquivalentTo("Maps/Island_W"))
                e.Edit(asset => {
                    var editor = asset.AsMap();
                    this.Monitor.VerboseLog("Editing IslandW...");
                    Map map = editor.Data;
            ...
            if (e.Name.IsEquivalentTo("Maps/FishShop") && CanEditFishShop)
                e.Edit(asset => {
                    var editor = asset.AsMap();
                    this.Monitor.VerboseLog("Editing FishShop...");
                    Map map = editor.Data;
                    map.Properties.Add("NPCWarp", "4 4 IslandSouth 19 43");
                });
            if (e.Name.IsEquivalentTo("Maps/IslandEast"))
                e.Edit(asset => {
                    var editor = asset.AsMap();
                    Map map = editor.Data;
                    this.Monitor.VerboseLog("Editing IslandEast...");
            ...
            if (e.Name.IsEquivalentTo("Maps/IslandShrine"))
                e.Edit(asset => {
                    var editor = asset.AsMap();
                    Map map = editor.Data;
                    this.Monitor.VerboseLog("Editing IslandShrine...");
            ...
            if (e.Name.IsEquivalentTo("Maps/IslandFarmCave"))
                e.Edit(asset => {
                    var editor = asset.AsMap();
                    this.Monitor.VerboseLog("Editing IslandFarmCave...");
                    Map map = editor.Data;
            ...
            if (e.Name.IsEquivalentTo("Maps/Island_W"))
                e.Edit(asset =>
                {
                    ...
                    Map map = editor.Data;
                    this.Monitor.VerboseLog("Editing Island_W...");
            ...
            if (Config.NPCDevan == true)
            {
                ...
                //saloon addition
                if (e.Name.IsEquivalentTo("Maps/Saloon") && Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is null)
                    e.Edit(asset =>
                    {
                        var editor = asset.AsMap();
                ...
                if (e.Name.IsEquivalentTo("Maps/Saloon") && Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is not null)
                    e.Edit(asset =>
                    {
                        var editor = asset.AsMap();
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Config.CustomChance >= RandomizedInt && IntKids >= 1)
                {
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Config.NPCDevan == true && IntKids <= 0)
                {
                    e.LoadFromModFile<Dictionary<string, string>>("assets/Devan/Schedule_Normal.json", AssetLoadPriority.Medium);
                };
                ...
                //married to leah
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Leah is 3 && Elliott is not 3)
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                ...
                //married to elliott
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Leah is not 3 && Elliott is 3)
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                ...
                //married to both
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Leah is 3 && Elliott is 3)
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Devan") && Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is not null)
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
            ...
            if (CanEditVanillas)
            {
                ...
                if (Config.CustomChance >= RandomizedInt)
                {
                    if (e.Name.IsEquivalentTo("Characters/schedules/Abigail") && Config.Allow_Abigail == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 16 9 0 \"Strings\\schedules\\Abigail:marriage_islandhouse\"/1100 IslandNorth 44 28 0 \"Strings\\schedules\\Abigail:marriage_loc1\"/a1750 IslandWest 39 41 0 \"Strings\\schedules\\Abigail:marriage_loc3\"/a2200 IslandFarmHouse 16 9 0";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Alex") && Config.Allow_Alex == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1000 IslandFarmHouse 19 6 0 \"Strings\\schedules\\Alex:marriage_islandhouse\"/1100 IslandWest 85 39 2 alex_lift_weights \"Strings\\schedules\\Alex:marriage_loc1\"/1300 IslandWest 69 77 2 alex_football /1500 IslandWest 64 83 2/a1900 IslandSouth 12 27 2 \"Strings\\schedules\\Alex:marriage_loc3\"/a2200 IslandFarmHouse 19 6 0";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Elliott") && Config.Allow_Elliott == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 9 9 2 \"Strings\\schedules\\Elliott:marriage_islandhouse\"/1100 IslandWest 102 77 2 elliott_read/1400 IslandWest 73 83 2 \"Strings\\schedules\\Elliott:marriage_loc2\"/a1900 IslandNorth 19 15 0 \"Strings\\schedules\\Elliott:marriage_loc3\"/a2200 IslandFarmHouse 9 9 2";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Emily") && Config.Allow_Emily == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 12 10 \"Strings\\schedules\\Emily:marriage_islandhouse\"/1100 IslandWest 53 52 2 \"Strings\\schedules\\Emily:marriage_loc1\"/1400 IslandWest 89 79 2 emily_exercise /1700 IslandWestCave1 3 6 1 \"Strings\\schedules\\Emily:marriage_loc3\"/a2200 IslandFarmHouse 12 10";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Haley") && Config.Allow_Haley == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 8 6 2 \"Strings\\schedules\\Haley:marriage_islandhouse\"/1100 IslandNorth 32 74 0 \"Strings\\schedules\\Haley:marriage_loc1\"/1400 IslandWest 76 12 2 haley_photo \"Strings\\schedules\\Haley:marriage_loc3\"/1900 IslandWest 80 45 2/a2200 IslandFarmHouse 8 6 0";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Harvey") && Config.Allow_Harvey == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 16 13 0 \"Strings\\schedules\\Harvey:marriage_islandhouse\"/1100 IslandFarmHouse 3 5 0 \"Strings\\schedules\\Harvey:marriage_loc1\"/1400 IslandWest 89 75 2 harvey_excercise/1600 IslandWest 88 14 2 harvey_read \"Strings\\schedules\\Harvey:marriage_loc3\"/a2100 IslandFarmHouse 16 13";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Krobus") && Config.Allow_Krobus == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 15 10 0 \"Characters\\Dialogue\\Krobus:marriage_islandhouse\"/1100 IslandFarmHouse 10 10 3 \"Characters\\Dialogue\\Krobus:marriage_loc1\"/1130 IslandFarmHouse 9 8 0/1200 IslandFarmHouse 9 11 2/1400 IslandWestCave1 8 8 0 \"Characters\\Dialogue\\Krobus:marriage_loc3\"/1500 IslandWestCave1 9 8 0/1600 IslandWestCave1 9 6 3/1900 IslandFarmCave 2 6 2/a2200 IslandFarmHouse 15 10 0";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Leah") && Config.Allow_Leah == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 21 13 1 \"Strings\\schedules\\Leah:marriage_islandhouse\"/1100 IslandNorth 50 25 0 leah_draw \"Strings\\schedules\\Leah:marriage_loc1\"/1400 IslandNorth 21 16 0/1600 IslandWest 89 72 2 leah_draw \"Strings\\schedules\\Leah:marriage_loc3\"/a2200 IslandFarmHouse 21 13 1";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Maru") && Config.Allow_Maru == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 18 15 2 \"Strings\\schedules\\Maru:marriage_islandhouse\"/1100 IslandWest 95 45 2/1400 IslandNorth 50 25 0 \"Strings\\schedules\\Maru:marriage_loc1\"/1700 IslandFieldOffice 7 8 0 \"Strings\\schedules\\Maru:marriage_loc3\"/a2200 IslandFarmHouse 18 15 2";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
                    ...
                    if (e.Name.IsEquivalentTo("Characters/schedules/Sebastian") && Config.Allow_Sebastian == true)
                        ...
                        {
                            IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                            data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 25 14 3 \"Strings\\schedules\\Sebastian:marriage_islandhouse\"/1100 IslandWest 8 9 0 sebastian_smoking/1400 IslandWestCave1 6 4 0 \"Strings\\schedules\\Sebastian:marriage_loc1\"/1600 IslandNorth 40 23 2 \"Strings\\schedules\\Sebastian:marriage_loc3\"/a2200 IslandFarmHouse 25 14 3";
                            data["marriage_Tue"] = "GOTO marriage_Mon";
                            data["marriage_Wed"] = "GOTO marriage_Mon";
                            data["marriage_Thu"] = "GOTO marriage_Mon";
                            data["marriage_Fri"] = "GOTO marriage_Mon";
            ...
            //SVE-specific
            if (Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is not null && Config.CustomChance >= RandomizedInt)
            {
                this.Monitor.VerboseLog("Adding SVE schedules");
                if (e.Name.IsEquivalentTo("Characters/schedules/Claire") && Config.Allow_Claire == true)
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                        data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 5 6 0 \"Characters\\Dialogue\\Claire:marriage_islandhouse\"/1100 IslandFarmHouse 17 12 2 Claire_Read \"Characters\\Dialogue\\Claire:marriage_loc1\"/1400 IslandEast 19 40 0 \"Characters\\Dialogue\\Claire:marriage_loc3\"/1600 IslandWest 87 78 2/a2150 IslandFarmHouse 5 6 0";
                        data["marriage_Tue"] = "GOTO marriage_Mon";
                        data["marriage_Wed"] = "GOTO marriage_Mon";
                        data["marriage_Thu"] = "GOTO marriage_Mon";
                        data["marriage_Fri"] = "GOTO marriage_Mon";
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Lance") && Config.Allow_Lance == true)
                    ...
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                        data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 13 13 1 \"Characters\\Dialogue\\Lance:marriage_islandhouse\"/1100 IslandNorth 37 30 0/1400 Caldera 24 23 2 \"Characters\\Dialogue\\Lance:marriage_loc2\"/1600 IslandSouthEast 21 28 2 \"Characters\\Dialogue\\Lance:marriage_loc3\"/a2150 IslandFarmHouse 13 13 1";
                        data["marriage_Tue"] = "GOTO marriage_Mon";
                        data["marriage_Wed"] = "GOTO marriage_Mon";
                        data["marriage_Thu"] = "GOTO marriage_Mon";
                        data["marriage_Fri"] = "GOTO marriage_Mon";
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Wizard") && Config.Allow_Magnus == true)
                    ...
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                        data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 26 6 0 \"Characters\\Dialogue\\Wizard:marriage_islandhouse\"/1100 IslandWest 38 38 0 \"Characters\\Dialogue\\Wizard:marriage_loc1\"/1400 {{Magnus_typeOfDay}}/1600 Caldera 22 23 0 \"Characters\\Dialogue\\Wizard:marriage_loc3\"/a2150 IslandFarmHouse 26 6 0";
                        data["marriage_Tue"] = "GOTO marriage_Mon";
                        data["marriage_Wed"] = "GOTO marriage_Mon";
                        data["marriage_Thu"] = "GOTO marriage_Mon";
                        data["marriage_Fri"] = "GOTO marriage_Mon";
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/Victor") && Config.Allow_Victor == true)
                    ...
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                        data["marriage_Mon"] = "620 FishShop 4 7 0/900 IslandSouth 1 11/940 IslandWest 77 43 0/1020 IslandFarmHouse 14 9 2 \"Characters\\Dialogue\\Victor:marriage_islandhouse\"/1100 IslandSouth 11 23 2 Victor_Wine2/1400 IslandFieldOffice 5 4 0 \"Characters\\Dialogue\\Victor:marriage_loc2\"/1600 IslandFarmHouse 19 5 2 Victor_Book2 \"Characters\\Dialogue\\Victor:marriage_loc3\"/a2150 IslandFarmHouse 14 9 2";
                        data["marriage_Tue"] = "GOTO marriage_Mon";
                        data["marriage_Wed"] = "GOTO marriage_Mon";
                        data["marriage_Thu"] = "GOTO marriage_Mon";
                        data["marriage_Fri"] = "GOTO marriage_Mon";
            ...
            if (Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is not null)
            {
                this.Monitor.Log("Adding SVE dialogue", LogLevel.Trace);
            ...
            foreach (ContentPackData cpd in CustomSchedule.Values)
            {
                ...
                {
                    this.Monitor.VerboseLog("asset => Commands.EditDialogue(cpd, asset, this.Monitor)");
                    e.Edit(asset => Commands.EditDialogue(cpd, asset, this.Monitor));
                    this.Monitor.LogOnce($"Added Ginger Island dialogue to {cpd.Spousename} data.", LogLevel.Debug);
                ...
                if (Config.CustomChance >= RandomizedInt)
                {
                    ...
                    string temp1 = "Characters/schedules/" + cpd.Spousename;
            ...
            //c2npc
            if (Helper.ModRegistry.Get("Loe2run.ChildToNPC") is not null && Config.CustomChance >= RandomizedInt && Config.Allow_Children == true && CNL is not null)
            {
                this.Monitor.Log("Child To NPC is in the mod folder. Adding compatibility...", LogLevel.Trace);
                if (e.Name.IsEquivalentTo("Characters/schedules/" + CNL[0]))
                {
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
                ...
                if (e.Name.IsEquivalentTo("Characters/schedules/" + CNL[1]))
                {
                    e.Edit(asset =>
                    {
                        IDictionary<string, string> data = asset.AsDictionary<string, string>().Data;
        ...
        private void OnAssetsInvalidated(object sender, AssetsInvalidatedEventArgs e)
        {
            ...
            //if file reloaded is part of framework:
            // remove from respective "files edited" list
            foreach (ContentPackData cpd in CustomSchedule.Values)
            {
                if (DialoguesEdited.Contains(cpd.Spousename) && e.NamesWithoutLocale.Equals($"Characters/Dialogue/{cpd.Spousename}"))
                {
                    this.Monitor.Log($"The Dialogue file for {cpd.Spousename} has been reloaded. Their name will be removed from DialoguesEdited.", LogLevel.Debug);
                    DialoguesEdited.Remove(cpd.Spousename);
                }
                if (SchedulesEdited.Contains(cpd.Spousename) && e.NamesWithoutLocale.Equals($"Characters/schedules/{cpd.Spousename}"))
                {
                    this.Monitor.Log($"The Schedule file for {cpd.Spousename} has been reloaded. Their name will be removed from DialoguesEdited.", LogLevel.Debug);
                    SchedulesEdited.Remove(cpd.Spousename);
                }
            }
        }
        private void OnSaveLoaded(object sender, SaveLoadedEventArgs e)
        {
            ...
            //reload standalone info
            Helper.GameContent.InvalidateCache("Maps/Island_W");
            Helper.GameContent.InvalidateCache("Maps/FishShop");

            //set leah and elliott friendship level- if bigger than 0, reload devan schedule
            Leah = owner.getFriendshipLevelForNPC("Leah");
            Elliott = owner.getFriendshipLevelForNPC("Elliott");
            if (Config.NPCDevan == true)
            {
                if (Leah is not 0)
                {
            ...
            //4h event
            SawDevan4H = owner.eventsSeen.Contains(110371000);
            if (owner.eventsSeen.Contains(110371000))
            {
                Helper.GameContent.InvalidateCache("Maps/Saloon");
            }


            //children information
            IntKids = owner.getChildrenCount();
            Children = owner.getChildren();
            foreach (var child in Children)
            {
                ChildNamesList.Add(child.Name);
            }
            if (IntKids is not 0)
            {
                CNL = ChildNamesList.ToArray();
                Helper.GameContent.InvalidateCache("Maps/IslandFarmHouse");
            }
            foreach (var child in Children)
            {
                if (IntKids >= 1 && Helper.ModRegistry.Get("Loe2run.ChildToNPC") is not null && Config.Allow_Children == true)
                {
        ...
        private void OnDayStarted(object sender, DayStartedEventArgs e)
        {
            ...
            var sgv = new SGIValues();
            RandomizedInt = Random.Next(1, 100);
            MatchesReloadCondition = Commands.ParseReloadCondition(PreviousDayRandom, Config.CustomChance, RandomizedInt);

            //framework
            foreach (string str in SchedulesEdited)
            {
                if (MatchesReloadCondition == true)
                {
            ...
            {
                //vanilla spouses
                if (Config.Allow_Abigail == true && sgv.CheckIfMarried("Abigail"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Abigail"); }
                if (Config.Allow_Alex == true && sgv.CheckIfMarried("Alex"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Alex"); }
                if (Config.Allow_Elliott == true && sgv.CheckIfMarried("Elliott"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Elliott"); }
                if (Config.Allow_Emily == true && sgv.CheckIfMarried("Emily"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Emily"); }
                if (Config.Allow_Haley == true && sgv.CheckIfMarried("Haley"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Haley"); }
                if (Config.Allow_Harvey == true && sgv.CheckIfMarried("Harvey"))
                {
                    Helper.GameContent.InvalidateCache("Characters/schedules/Harvey");
                    Helper.GameContent.InvalidateCache("Characters/Harvey");
                }
                if (Config.Allow_Krobus == true && sgv.CheckIfMarried("Krobus"))
                {
                    Helper.GameContent.InvalidateCache("Portraits/Krobus");
                    Helper.GameContent.InvalidateCache("Characters/Krobus");
                    Helper.GameContent.InvalidateCache("Characters/schedules/Krobus");
                }
                if (Config.Allow_Leah == true && sgv.CheckIfMarried("Leah"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Leah"); }
                if (Config.Allow_Maru == true && sgv.CheckIfMarried("Maru"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Maru"); }
                if (Config.Allow_Penny == true && sgv.CheckIfMarried("Penny"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Penny"); }
                if (Config.Allow_Sam == true && sgv.CheckIfMarried("Sam"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Sam"); }
                if (Config.Allow_Sebastian == true && sgv.CheckIfMarried("Sebastian"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Sebastian"); }
                if (Config.Allow_Shane == true && sgv.CheckIfMarried("Shane"))
                { Helper.GameContent.InvalidateCache("Characters/schedules/Shane"); }
            }
            ...
            if (MatchesReloadCondition == true)
            {
                if (Helper.ModRegistry.Get("FlashShifter.StardewValleyExpandedCP") is not null)
                {
                    if (Config.Allow_Claire == true && sgv.CheckIfMarried("Claire"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Claire"); }
                    if (Config.Allow_Lance == true && sgv.CheckIfMarried("Lance"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Lance"); }
                    if (Config.Allow_Magnus == true && sgv.CheckIfMarried("Magnus"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Wizard"); }
                    if (Config.Allow_Olivia == true && sgv.CheckIfMarried("Olivia"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Olivia"); }
                    if (Config.Allow_Sophia == true && sgv.CheckIfMarried("Sophia"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Sophia"); }
                    if (Config.Allow_Victor == true && sgv.CheckIfMarried("Victor"))
                    { Helper.GameContent.InvalidateCache("Characters/schedules/Victor"); }
                }
                //
                if (Config.NPCDevan == true && IntKids >= 1)
                {
                    Helper.GameContent.InvalidateCache("Characters/schedules/Devan");
                }
                if (IntKids >= 1 && Helper.ModRegistry.Get("Loe2run.ChildToNPC") is not null && Config.Allow_Children == true)
        ...
        {
            if (e.NewTime is 1710)
            {
                CanEditFishShop = false;
                Helper.GameContent.InvalidateCache("Maps/FishShop");
            }
        ...
        //    internal (can only be accessed by ModEntry.cs)
        ...
        internal void SGI_Reset(string command, string[] args)
        {
            ...
            if (!args.Any() || args[0] is "help" || args[0] is "h")
            {
            ...
            else
            {
                ...
                if (args[0] is "dialogues" || args[0] is "d")
                {
                    foreach (string str in CustomSchedule.Keys)
                    {
                        Helper.GameContent.InvalidateCache($"Characters/Dialogue/{str}");
                        ResetCounter++;
                    }
                    this.Monitor.Log($"Reloaded {ResetCounter} dialogues.", LogLevel.Info);
                    ResetCounter = 0;
                }
                if (args[0] is not "schedules" && args[0] is not "s" && args[0] is not "dialogues" && args[0] is not "d")
                {
        ...
        //variables
        internal string[] CNL { get; private set; }
        internal List<string> DaysOfWeek = new List<string>();
        internal List<string> SchedulesEdited = new List<string>();
        internal List<string> DialoguesEdited = new List<string>();
        ...
        internal bool MatchesReloadCondition;
        internal bool CCC;
        internal bool SawDevan4H = false;
        internal bool CanEditVanillas = true;
        internal bool CanEditFishShop;
